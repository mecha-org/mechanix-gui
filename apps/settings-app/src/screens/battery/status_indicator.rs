...
impl StatusIndicator {
    ...
    pub fn handle_on_drag(
        ...
        println!("StatusIndicator::handle_on_drag() {:?}", grid);
        return Some(value);
    }
...
impl Component for StatusIndicator {
    ...
    fn render_hash(&self, hasher: &mut ComponentHasher) {
        ...
        // println!("StatusIndicator::render_hash() {:?}", hasher.finish());
    }
    ...
    fn on_drag_start(&mut self, event: &mut Event<event::DragStart>) {
        println!("StatusIndicator::on_drag_start()");
        event.stop_bubbling();
    }

    fn on_touch_drag_start(&mut self, event: &mut Event<event::TouchDragStart>) {
        println!("StatusIndicator::on_touch_drag_start()");
        event.stop_bubbling();
    }
    ...
    fn on_touch_drag(&mut self, event: &mut Event<event::TouchDrag>) {
        ...
        if let Some(value) = self.handle_on_drag(
            ...
            println!("StatusIndicator::on_touch_drag() value {:?}", value,);
            if let Some(f) = &self.on_slide {
                event.emit(f(value));
            }
        }
