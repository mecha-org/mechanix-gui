...
class LockScreenTimeout extends StatefulWidget {
  const LockScreenTimeout({super.key});

  @override
  State<LockScreenTimeout> createState() => _LockScreenTimeoutState();
}

class _LockScreenTimeoutState extends State<LockScreenTimeout> {
  @override
  void initState() {
    super.initState();
  }
  ...
  void onChanged(SelectOption option) {
    if (option.value is DisplayScreenOffTime) {
      context
          .read<DisplayBloc>()
          .add(SetLockScreenTimeoutEvent(displayTime(option.value)));
    }
