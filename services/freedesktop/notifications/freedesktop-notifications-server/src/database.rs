...
pub async fn add_notification_to_db(id: u32, notification: &Notification) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn get_all_notifications_from_db() -> Result<HashMap<u32, Notification>, Box<dyn std::error::Error>> {
    let db = sled::open(DATABASE_PATH)?;
    ...
    for result in db.iter() {
        ...
        let id_bytes: [u8; 4] = key.as_ref().try_into()
