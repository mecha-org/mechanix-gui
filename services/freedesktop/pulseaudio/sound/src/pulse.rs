...
impl PulseHandle {
    ...
    pub fn new() -> Result<Self, PulseInitError> {
        ...
        let (to_pulse, mut to_pulse_recv) = mpsc::channel(50);
        let (from_pulse, mut from_pulse_recv) = mpsc::channel(50);
                ...
                rt.block_on(async {
                    match PulseServer::connect().and_then(|s| s.init()) {
                                    ...
                                    Message::GetSinks => {
                                        ...
                                        match server.get_sinks() {
                                            Ok(sinks) => {
                                                ...
                                                if let Err(err) = from_pulse.send(sinks).await{
                                            ...
                                            Err(e) => {
                                                ...
                                                if let Err(err) = from_pulse.send(Message::GetSinks).await{
                        ...
                        Err(err) => {
                            ...
                            let _ = init_tx.send(Err(PulseInitError::InitFailed(format!("{:?}", err))));
                        }
                    }
                });
            });
        ...
        match init_rx.try_recv() {
            Ok(Ok(())) => Ok(Self { to_pulse, from_pulse }),
            Ok(Err(e)) => Err(e),
