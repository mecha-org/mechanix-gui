...
pub struct ThemeConfigs {
    pub theme: HomescreenTheme, // Theme configs
}

impl Default for HomescreenTheme {
    fn default() -> Self {
        Self {
...
/// # Font Settings
///
/// Declares all the fonts needed for the homescreen with their
...
/// # Reads Theme YML
///
/// Reads the `settings.yml` and parsers to HomescreenTheme
///
/// **Important**: Ensure all fields are present in the yml due to strict parsing
pub fn read_theme_yml() -> Result<HomescreenTheme> {
    let mut file_path = PathBuf::from(
        std::env::var("MECHA_HOMESCREEN_THEME_PATH").unwrap_or(String::from("theme.yml")),
    ...
    let theme_file_handle = match File::open(file_path) {
        ...
        Err(e) => {
            bail!(HomescreenError::new(
                HomescreenErrorCodes::ThemeReadError,
    ...
    let config: ThemeConfigs = match serde_yaml::from_reader(theme_file_handle) {
        ...
        Err(e) => {
            bail!(HomescreenError::new(
                HomescreenErrorCodes::ThemeParseError,
