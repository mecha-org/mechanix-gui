...
pub struct ThemeConfigs {
    pub theme: AppDockThemeTheme, // Theme configs
}
...
/// # Reads Theme YML 
/// 
/// Reads the `theme.yml` and parsers to AppDockThemeTheme
/// 
/// **Important**: Ensure all fields are present in the yml due to strict parsing
pub fn read_theme_yml() -> Result<AppDockThemeTheme> {
    let mut file_path = PathBuf::from(std::env::var("MECHA_APP_DOCK_THEME_PATH")
    ...
    let theme_file_handle = match File::open(file_path) {
        ...
        Err(e) => {
            bail!(AppDockError::new(
                AppDockErrorCodes::ThemeReadError,
    ...
    let config: ThemeConfigs = match serde_yaml::from_reader(theme_file_handle) {
        ...
        Err(e) => {
            bail!(AppDockError::new(
                AppDockErrorCodes::ThemeParseError,
