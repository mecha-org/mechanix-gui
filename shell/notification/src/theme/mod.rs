...
pub struct ThemeConfigs {
    pub theme: NotificationTheme, // Theme configs
}
...
/// # Font Settings
///
/// Declares all the fonts needed for the notification with their
...
/// # Reads Theme YML
///
/// Reads the `settings.yml` and parsers to NotificationTheme
///
/// **Important**: Ensure all fields are present in the yml due to strict parsing
pub fn read_theme_yml() -> Result<NotificationTheme> {
    let mut file_path = PathBuf::from(
        std::env::var("MECHANNIX_NOTIFICATION_THEME_PATH").unwrap_or(String::from("theme.yml")),
    ...
    let theme_file_handle = match File::open(file_path) {
        ...
        Err(e) => {
            bail!(NotificationError::new(
                NotificationErrorCodes::ThemeReadError,
    ...
    let config: ThemeConfigs = match serde_yaml::from_reader(theme_file_handle) {
        ...
        Err(e) => {
            bail!(NotificationError::new(
                NotificationErrorCodes::ThemeParseError,
