...
impl AppManagerService {
    ...
    pub async fn run(
        ...
        app_switcher_sender: mpsc::Sender<AppMessage>
    ) {
        ...
        for tl in toplevels.into_iter().rev() {
            let (tx, rx) = oneshot::channel();
            let _ = toplevel_msg_tx.send(ToplevelMessage::GetToplevelMeta {
                key: tl,
                reply_to: tx,
            }).await;
                ...
                event = toplevel_event_rx.recv() => {
                    ...
                    match event.unwrap() {
                        ...
                        ToplevelEvent::Done {
                            ...
                            let _ = app_switcher_sender.send(AppMessage::AppsUpdated  { app_id: possible_app_id, apps: formatted_apps , active_apps_count }).await;
                        }
    ...
    pub async fn activate_app(&self, app_id: &str) -> Result<bool> {
        ...
        let instance = match instance_op {
            Some(v) => v,
            None => { bail!("app is not having any top level key") }
        };
    ...
    pub fn add_app(
        ...
        let mut instances: IndexMap<ToplevelKey, AppInstanceState> = match
            self.apps.get_key_value(&app_id)
        {
            Some((_, v)) => v.clone(),
            None => IndexMap::new(),
        };
    ...
    pub async fn set_instance_fullscreen(&mut self, instance: ToplevelKey) -> Result<bool> {
        let (tx, rx) = oneshot::channel();
        let _ = self.top_level_sender.as_ref().unwrap().send(ToplevelMessage::SetFullscreen {
            key: instance,
            reply_to: tx,
        }).await;
