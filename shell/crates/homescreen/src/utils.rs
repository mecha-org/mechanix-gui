...
pub fn grid_bounds_to_pixels(
    ...
    let x = padding.left + (grid_bounds.origin.x as f32 * cell_with_gap_h);
    let y = padding.top + (grid_bounds.origin.y as f32 * cell_with_gap_v);

    // Calculate widget size (cells * cell_size + (cells - 1) * gap)
    let width = grid_bounds.size.width as f32 * cell_width
        + (grid_bounds.size.width.saturating_sub(1)) as f32 * gap.horizontal;

    let height = grid_bounds.size.height as f32 * cell_height
        + (grid_bounds.size.height.saturating_sub(1)) as f32 * gap.vertical;
...
pub fn pixel_position_to_grid_bounds(
    ...
    let adjusted_x = (relative_x - padding.left).max(0.0);
    let adjusted_y = (relative_y - padding.top).max(0.0);
...
pub fn pixel_bounds_to_closest_grid_bounds(
    ...
    let adjusted_x = (origin_x - padding.left).max(0.0);
    let adjusted_y = (origin_y - padding.top).max(0.0);
