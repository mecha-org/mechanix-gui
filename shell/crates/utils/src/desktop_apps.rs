...
use bevy::{asset::AssetPath, ecs::system::SystemId, prelude::*};
...
fn get_desktop_apps(app: &mut App) -> Vec<DesktopApp> {
    ...
    let mut apps: Vec<DesktopApp> = entries
        ...
        .filter_map(|entry| {
            ...
            let on_click = app.world_mut().register_system(move || {
...
impl Plugin for DesktopAppsPlugin {
    fn build(&self, app: &mut App) {
        let desktop_apps = DesktopApps::new(app);
        app.insert_resource(desktop_apps);
    }
}
