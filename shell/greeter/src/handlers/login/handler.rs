...
use crate::{AppMessage, Greeter, LoginHandlerEvents, Prompt};
...
impl LoginHandler {
    ...
    pub async fn login(&mut self, input: String, sender: &Sender<AppMessage>) -> Result<bool> {
    ...
    async fn start_session(&mut self, sender: &Sender<AppMessage>) {
        let environment = vec![];
    ...
    fn display_error(&mut self, sender: &Sender<AppMessage>, display_text: &str, log_text: &str) {
        error!("{log_text}");

        let error_message = String::from(display_text.clone());
        sender.send(AppMessage::LoginEvents(LoginHandlerEvents::ShowErr(
            error_message,
        )));
        // sleep(Duration::from_secs(5)).await;
        sender.send(AppMessage::LoginEvents(LoginHandlerEvents::ClearErr));
    }

    async fn create_session(&mut self, sender: &Sender<AppMessage>, username: String) {
    ...
    pub async fn handle_greetd_response(
        ...
        match response {
            ...
            Response::AuthMessage {
                ...
                match auth_message_type {
                    AuthMessageType::Secret => {
                        ...
                        sender.send(AppMessage::LoginEvents(LoginHandlerEvents::Prompts(
                            Prompt::Password {
                    ...
                    AuthMessageType::Visible => {
                        ...
                        sender.send(
                            async move {
                                AppMessage::LoginEvents(LoginHandlerEvents::Prompts(
                                    Prompt::Captcha {
                                        message: auth_message.clone(),
                                    },
                                ))
            ...
            Response::Error {
                ...
                if let ErrorType::AuthError = error_type {
                    ...
                    sender.send(AppMessage::LoginEvents(LoginHandlerEvents::AuthError));
                }
        ...
        sender.send(
            async move {
                ...
                AppMessage::LoginEvents(LoginHandlerEvents::HandleGreetdResponse(response))
            }
            .await,
        );
    }

    async fn send_input(&mut self, sender: &Sender<AppMessage>, input: String) {
